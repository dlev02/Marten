...
## September 14–15, 2026 · Forecast investing, background imports, net worth at scale

Work started from a real household that had just imported about 6,100 Monarch transactions and 11,900 daily balance rows, which exposed three defects: the net worth chart's ALL range failed with a server error, imported accounts stayed at $0, and the transactions page re-read every row on each visit while the Convex database bandwidth sat at the free-plan ceiling.

- Root causes, confirmed in code and by targeted tests: `balanceHistory` returned every balance row for the range, and Convex values hold at most 8,192 array items, so ranges with more rows than that failed; `importBalances` never touched an account's current balance; `useTransactions` auto-loaded every page (200 rows at a time) on the Transactions page and the paginated subscription was dropped and re-created on every visit.
- Fixes: a server-side `netWorthHistory` query returns one summed point per day (or per sample step for long ranges, with per-account lookups bounded to about 1,500 index reads) plus per-account series for the sparklines; the Dashboard and Accounts charts use it and the balance export fetches rows on demand. Balance imports now set a manually tracked account's current balance from its newest imported day. The app wraps Convex in `ConvexQueryCacheProvider` (10-minute retention) through a shared `src/lib/convex.ts` hook module, and the transactions list pages as you scroll unless a client-side filter, sort, total or export needs every row.
- Imports run as a server job: rows upload as one JSON file, `internal.imports.run` saves batches through the same helpers the dialog used and records progress; the dialog can be closed and a status card follows the job. Tests cover a 205-row job, a balances job that sets manual balances, a malformed upload, retry skipping, ownership, and the one-job-at-a-time rule.
- Forecast version 2 inputs add **Invest each month** and **Retirement contributions**; version 1 scenarios still run with their original meaning and are upgraded on load. Engine tests check cash retention, capped investing, payroll growth, monthly money conservation, and the upgrade helper.
- Category merge (`settings.mergeCategories`, agent `merge_categories`) is covered by a test moving main categories, split lines, rule actions and conditions, then deleting the source. Imported categories now land in a suggested starter group, with tests for ten Monarch-style names.
- Automated: 365 tests across 49 files ran; 364 passed. The one failure, `convex/simplefin.test.ts` ("second sync updates two rows"), also fails on the previous commit without these changes and was left alone. Typecheck, ESLint and the production build passed.
- Development deployment: `npx convex dev --once` pushed the schema (new `importJobs` table and index) and functions three times during the pass. Convex reported the team above Free plan limits during each push; no production deploy was run from here (Netlify deploys production on push).
- Browser, fictional demo workspace at 1280×900 in dark and light: Dashboard net worth (6M), Accounts with ALL selected (no console errors, full curve), transactions list showing "Newest 100 · scroll for more", loading the next page on scroll and reaching "250 transactions", and returning from the Dashboard without a loading screen. Forecast showed the Investing section with the observed unspent amount, the collapsible income/starting-funds summaries, info tips in place of the removed notes, and the Invested ledger column. Reports toolbar rendered "By category" at the same height as the date and Filters buttons; Preferences showed the initials tile first in Choose avatar; the category dialog opened Merge into another category and its target picker.
- Not verified in a browser: the import job's progress card end to end (the Browser pane cannot attach a file; the flow is covered by backend tests), the Plaid step's back link (not reachable in a demo), and the live production data after deploy. The first Vite start after adding the cache provider produced a duplicate React copy until the modules were added to `optimizeDeps.include`; the config change is committed.
