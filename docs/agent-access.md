...
| Organization          | Category groups, categories with group kind, tags, merchant pages or `merchantSearch`, ordered rules with names | Rename merchants, create and edit categories and tags, merge a duplicate category into another of the same kind (`merge_categories`), save rules, apply a rule to existing rows in bounded pages |
