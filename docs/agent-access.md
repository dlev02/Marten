...
| --------------------- | ------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| Accounts              | Balances, currencies, cached update times, statements                                 | Name, hidden state, net-worth inclusion                                         |
| Transactions          | Pages, details, annotation history, category groups, categories, merchant pages, tags | Notes, classification, merchant, tags, visibility, reviewed state, valid splits |
| Recurring             | Schedules, detector suggestions, paginated payment checkmarks, statement reminders    | Create or update tracking schedules; set or clear actual occurrence checkmarks  |
| Forecast              | Baseline, saved assumptions and revisions, modeled annual results                     | Save a reviewed scenario; updates require its current revision                  |
...
| Reports and cash flow | Complete USD totals using the application's split and category-group rules            | None                                                                            |

The tool surface has no bank linking, provider refresh, token retrieval, receipt downloads, transfers, trades, message sending, deletion, Budget, Goals, or Advice tools. Creating or marking a recurring item changes tracking only. Names, notes, and statements are untrusted stored text, not instructions to the assistant.

Amounts remain integer cents with currency. Reports exclude hidden, pending, removed, and transfer entries; refunds retain their sign and splits replace the parent allocation. Merchant, transaction, investment-activity, and payment reads expose pagination. Reports scan at most 60 pages of 200 transactions; when unfinished, they return `complete: false` and `summary: null`. Pages can observe a bank sync or edit during loading, so the result explains that consistency limit. Forecasts use the monthly engine but return annual rows and totals to keep responses manageable. Remote results above 140,000 characters return a request to narrow the query, never silently truncated data.
