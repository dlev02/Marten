...
> Historical concept specification. The maintained implementation reference is
> [root DESIGN.md](../../DESIGN.md). Values and component decisions below record
> the earlier concept/refinement phase and may differ from today's source.
> Preserve this document for provenance; use DESIGN.md for new work.
...
- Icons: Lucide outline, 20px in navigation, 16px controls, 1.7px strokes. Category illustrations use the original Marten pictogram library, with coordinated light/dark palettes and an optional system-emoji appearance; merchant artwork comes from the generated local catalog described in [assets](../assets.md). Home, credit-card, list, bar-chart, pie-chart, calendar, search, settings, moon, chevrons, arrows, paperclip, eye, check-circle, plus, grip. Hover shades and keyboard focus ring.
...
- Motion: 160ms hover/fade, 240ms drawer slide, 180ms theme crossfade, and 240ms sidebar snapshot transition. Net-worth charts reveal left-to-right (about 680ms, ease-out) when they first load and when the range (3M/6M/YTD/1Y/All) changes, using a clip over the plot so axes stay put; the reveal never replays on resize or live balance updates, and sparklines skip it. Other financial charts avoid reveal animations while adjusting data, and net-worth paths render immediately on resize so the prior path cannot interpolate across a newly positioned axis. Reduced motion disables nonessential movement, including the reveal.
- Responsive: The expanded sidebar remains 236px across desktop widths (76px when manually collapsed); 1000px two-column panels simplify, 760px sidebar becomes offcanvas, tables scroll within region and preserve amount visibility, drawers full viewport on narrow screens. Controls wrap into useful rows.
...
The [Mobbin refinement review](mobbin-review.md) records the verified authentication, empty-state, settings, command-search, date-filter and assumption-control references.
