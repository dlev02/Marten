...
export async function listForecastsForUser(ctx: UserRead) {
...
export const list = userQuery({
  args: {},
  handler: listForecastsForUser,
});

export async function saveForecastForUser(
  ctx: UserMutationCtx,
  args: {
...
export async function forecastBaselineForUser(
  ctx: UserRead,
  ...
  const excludedDebtCents = accountRows
    .filter(
      (a) => a.currency === "USD" && (a.kind === "credit" || a.kind === "loan"),
    )
    .reduce((total, a) => total + a.balanceCents, 0);
...
export const baseline = userQuery({
  ...
  handler: forecastBaselineForUser,
});
